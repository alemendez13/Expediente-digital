document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderHistoryTables(history) {
        ...
        if (Object.keys(labData).length > 0) {
            labTableHtml = `<table class="w-full text-sm text-left text-gray-600 history-table"><thead class="text-xs text-gray-700 uppercase"><tr><th scope="col" class="px-6 py-3">Estudio</th><th scope="col" class="px-6 py-3">Fecha</th><th scope="col" class="px-6 py-3">Resultado</th></tr></thead><tbody>`;
            for (const studyName in labData) {
                labData[studyName].forEach((entry, index) => {
                    labTableHtml += `<tr class="bg-white border-b hover:bg-gray-50">${index === 0 ? `<th scope="row" class="px-6 py-4 font-bold text-gray-900 capitalize" rowspan="${labData[studyName].length}">${studyName}</th>` : ''}<td class="px-6 py-4">${entry.date}</td><td class="px-6 py-4">${entry.result}</td></tr>`;
                });
            }
        ...
        if (Object.keys(gabineteData).length > 0) {
            gabineteTableHtml = `<table class="w-full text-sm text-left text-gray-600 history-table"><thead class="text-xs text-gray-700 uppercase"><tr><th scope="col" class="px-6 py-3">Estudio</th><th scope="col" class="px-6 py-3">Fecha</th><th scope="col" class="px-6 py-3">Resultado</th></tr></thead><tbody>`;
            for (const studyName in gabineteData) {
                gabineteData[studyName].forEach((entry, index) => {
                    gabineteTableHtml += `<tr class="bg-white border-b hover:bg-gray-50">${index === 0 ? `<th scope="row" class="px-6 py-4 font-bold text-gray-900 capitalize" rowspan="${gabineteData[studyName].length}">${studyName}</th>` : ''}<td class="px-6 py-4">${entry.date}</td><td class="px-6 py-4">${entry.result}</td></tr>`;
                });
            }
    ...
    function initializeCalculations() {
        ...
        function updateIpaqScore() {
            ...
            if (interpretationInput) {
                if (totalScore < 163) interpretationInput.value = "Nivel Bajo o Inactivo";
                else if (totalScore >= 163 && totalScore < 1500) interpretationInput.value = "Nivel Moderado";
                else interpretationInput.value = "Nivel Alto";
            }
        }
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            if (actividadSelect && intensidadInput) {
                actividadSelect.addEventListener('change', () => {
                    intensidadInput.value = intensityMap[actividadSelect.value] || '';
                    updateIpaqScore();
                });
            }
            if(diasInput) diasInput.addEventListener('input', updateIpaqScore);
        }
        updateIpaqScore();
        ...
        function updateAlimentacionScore() {
            ...
            cribadoFields.forEach(field => {
                ...
                if (select && scoreInput) {
                    const score = scoreMapAlimentacion[select.value] ?? 0;
            ...
            if (resultadoInput) {
                let interpretation = 'Bajo consumo saludable';
                if (totalScore >= 3 && totalScore <= 5) interpretation = 'Moderado consumo saludable';
                else if (totalScore >= 6) interpretation = 'Alto consumo saludable';
    ...
    function initializePatologicosComponent() {
        ...
        window.toggleContent = function(checkboxElement) {
            const detailsContainer = checkboxElement.closest('.disease-row').querySelector('.disease-details');
            if (detailsContainer) detailsContainer.classList.toggle('hidden', !checkboxElement.checked);
        }
        ...
        chronicDiseasesConfig.forEach(system => {
            ...
            system.diseases.forEach(disease => {
                let detailFields = `<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-3">${disease.isOther ? `<div class="md:col-span-3"><label class="form-label">Especificar</label><textarea name="app_${disease.id}_especificar" class="form-textarea" rows="1"></textarea></div>` : ''}<div><label class="form-label">Edad/Año de diagnóstico</label><input type="text" name="app_${disease.id}_dx_edad" class="form-input"></div><div><label class="form-label">Tratamientos previos</label><input type="text" name="app_${disease.id}_tx_previo" class="form-input"></div><div><label class="form-label">Tratamiento actual</label><input type="text" name="app_${disease.id}_tx_actual" class="form-input"></div><div class="md:col-span-2"><label class="form-label">Evolución</label><input type="text" name="app_${disease.id}_evolucion" class="form-input"></div><div><label class="form-label">Complicaciones</label><input type="text" name="app_${disease.id}_complicaciones" class="form-input"></div></div>${disease.questionnaire ? questionnairesHTML[disease.questionnaire] || '' : ''}`;
                let subDiseasesHtml = '';
                if(disease.subDiseases) {
                    subDiseasesHtml = disease.subDiseases.map(sub => `<div class="pl-4"><label class="flex items-center space-x-3 cursor-pointer"><input type="checkbox" name="app_${sub.toLowerCase()}_presente" class="form-checkbox h-4 w-4 text-cyan-500 rounded"><span class="font-normal text-gray-600">${sub}</span></label></div>`).join('');
                }
                diseasesHtml += `<div class="disease-row border-t pt-3"><label class="flex items-center space-x-3 cursor-pointer"><input type="checkbox" name="app_${disease.id}_presente" class="form-checkbox h-5 w-5 text-cyan-600 rounded" onclick="toggleContent(this)"><span class="font-medium text-gray-800">${disease.name}</span></label>${subDiseasesHtml}<div class="disease-details hidden pl-8 pt-2">${detailFields}</div></div>`;
            });
            html += `<div class="border rounded-md"><div class="section-header p-2 bg-gray-50 hover:bg-gray-100" onclick="toggleAccordion(this)"><h4 class="font-semibold text-gray-700">${system.name}</h4><svg class="w-5 h-5 transform transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg></div><div class="p-4 hidden"><div class="space-y-4">${diseasesHtml}</div></div></div>`;
        });
